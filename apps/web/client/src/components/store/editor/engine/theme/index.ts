...
export class ThemeManager {
  ...
  async updateTailwindColorConfig(
    ...
    try {
      ...
      const camelCaseName =
        newName === DEFAULT_COLOR_NAME ? newName : camelCase(newName);

      if (originalKey) {
        const [parentKey, keyName] = originalKey.split("-");

        const isDefaultColor =
          parentKey && colors[parentKey as keyof typeof colors];
        if (isDefaultColor) {
  ...
  getConfigPath(): {
    ...
    const configPath =
      list.find((file: string) => file.includes("tailwind.config")) ?? null;
    const cssPath =
      list.find((file: string) => file.includes("globals.css")) ?? null;
  ...
  async scanTailwindConfig() {
    try {
      ...
      const configContent =
        await this.editorEngine.sandbox.readFile(configPath);
      if (!configContent) {
  ...
  async updateTailwindColorVariable(
    ...
    if (!keyName) {
      ...
      if (keyName === DEFAULT_COLOR_NAME) {
        ...
        newCssVarName =
          newName !== keyName ? `${parentKey}-${newName}` : originalName;
      }
    }
  ...
  async initializeTailwindColorContent(): Promise<ColorUpdate | null> {
    ...
    const files = await this.editorEngine.sandbox.readFiles([
      configPath,
      cssPath,
    ]);
            ...
            const newClasses = oldClasses.map((currentClass) => {
              for (const { oldClass, newClass } of replacements) {
                ...
                if (oldClassPattern.test(currentClass)) {
                  hasChanges = true;
                  return newClass
                    ? currentClass.replace(oldClass, newClass)
                    : "";
                }
              }
              return currentClass;
