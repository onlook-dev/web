...
export class TextEditingManager {
    ...
    async start(el: DomElement, frameView: WebFrameView): Promise<void> {
        try {
            const isEditable = await frameView.isChildTextEditable(el.oid ?? '') as boolean | null;
            if (isEditable !== true) {
                toast({
    ...
    async edit(newContent: string): Promise<void> {
        ...
        try {
            ...
            const domEl = await frameData.view.editText(this.targetDomEl.domId, newContent) as DomElement | null;
            if (!domEl) {
    ...
    private async handleEditedText(domEl: DomElement, newContent: string, frameView: WebFrameView): Promise<void> {
        try {
    ...
    async editSelectedElement(): Promise<void> {
        ...
        try {
            ...
            const domEl = await frameData.view.getElementByDomId(selectedEl.domId, true) as DomElement;
            if (!domEl) {
                return;
            }
